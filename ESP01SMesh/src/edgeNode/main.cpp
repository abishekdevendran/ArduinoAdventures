  ...
  void mqttCallback(char* topic, uint8_t* payload, unsigned int length) {
    ...
    if(targetStr == "gateway")
    {
      if(msg == "getNodes")
      {
        ...
        // split targetStr into 2 strings with : as the delimiter
        int colonIdx = msg.indexOf(':');
        if(colonIdx>=0 && colonIdx<msg.length()){
          String app = msg.substring(0, colonIdx);
          String sens = msg.substring(colonIdx+1);
          Serial.printf("App: %s, Sensor: %s\n", app.c_str(), sens.c_str());
          //add to sensordata json
          sensorData[app]=sens;
          //add to sensors string without recurring characters
          int flag = 0;
          for(int i=0;i<sens.length();i++){
            if(!(sensors.indexOf(sens[i])>=0 && sensors.indexOf(sens[i])<sensors.length())){
              Serial.printf("Appending sensor %c\n", sens[i]);
              sensors+=sens[i];
              flag++;
            }
          }
          if(flag){
            //broadcast new sensor to every node
            mesh.sendBroadcast(sensors,false);
          }
        }
      }
    }
